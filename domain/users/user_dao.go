...
import (
	"errors"
	"fmt"
	"github.com/katsun0921/bookstore_users-api/datasources/mysql/users_db"
	"github.com/katsun0921/bookstore_users-api/logger"
	"github.com/katsun0921/bookstore_users-api/utils/mysql_utils"
...
func (user *User) Save() *rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to get last insert creating a new user", err)
		return rest_errors.NewInternalServerError("error when trying to save user", errors.New("database error"))
	}
...
func (user *User) FindByEmailAndPassword() *rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to find by email and password", errors.New("database error"))
	}
	defer stmt.Close()
